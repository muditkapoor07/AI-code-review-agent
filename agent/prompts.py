...
RULES: blocking_issues = [] always (leave it empty — it is computed automatically). fix.code and fix.description = strings (never null). severity must be: critical/high/medium/low/info. category must be: bug/security/performance/code_quality (underscore, not space).
